...
import { currentAllStaffExceptAssistant, currentAllStaffExceptTechnician, currentManagerAndComanager, currentManagerAndDentist, currentManagerAndTechnician} from "@/lib/auth";
...
export const validateManagerAndComanager = async (
  companyId: string
): Promise<ValidateManagerResult> => {
  if (!companyId) {
    return NextResponse.json(
  ...
  const member = await currentManagerAndComanager();
  if (!member) {
    return NextResponse.json(
      { error: "คุณไม่มีสิทธิเข้าถึงข้อมูลน้ี" },
      { status: 403 }
